    ...
    public class WebSocketHandler
    {
        ...
        public async Task HandleWebSocketConnection(HttpContext context)
        {
            if (context.WebSockets.IsWebSocketRequest)
            {
                ...
                _logger.Log(LogLevel.Information, $"Client connected: {clientId}");
        ...
        private async Task ReceiveMessagesAsync(WebSocket webSocket, string clientId)
        {
            ...
            do
            {
                ...
                _logger.Log(LogLevel.Information, $"ReceiveMessagesAsync clientId: {clientId} Message : {message}");
            ...
            _logger.Log(LogLevel.Information, $"Client disconnected: {clientId}");
           ...
           if (_clients.TryGetValue(clientId, out var webSocket) && webSocket.State == WebSocketState.Open)
            {
                ...
                _logger.Log(LogLevel.Information, $"SendMessageToClient clientId: {clientId} message {message}");
            }
        }
        ...
        public async Task BroadcastMessage(string message)
        {
            ...
            foreach (var client in _clients)
            {
                ...
                {
                    _logger.Log(LogLevel.Information, $"BroadcastMessage  message {message}");
